...
class AdminProfileTab extends StatelessWidget {
  const AdminProfileTab({super.key, required this.user});
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      child: ListView(
        ...
        children: [
          HeroHeader(name: user['name'] as String, subtitle: 'Pengaturan Sistem', icon: Icons.settings),
          ...
          ActionTile(icon: Icons.local_shipping, title: 'Manajemen Pesanan', subtitle: 'Perbarui status pesanan', onTap: () => push(context, OrdersScreen(user: user))),
          ActionTile(icon: Icons.bar_chart, title: 'Laporan dan Dashboard', subtitle: 'Ringkasan konsultasi dan transaksi', onTap: () => push(context, ReportsScreen(user: user))),
          ActionTile(icon: Icons.notifications, title: 'Notifikasi', subtitle: 'Lihat pemberitahuan sistem', onTap: () => push(context, NotificationsScreen(user: user))),
        ],
      ),
    );
  }
