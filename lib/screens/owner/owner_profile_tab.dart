...
class OwnerProfileTab extends StatelessWidget {
  const OwnerProfileTab({super.key, required this.user});
    ...
    return AppScaffold(
      ...
      child: ListView(
        ...
        children: [
          ...
          ActionTile(icon: Icons.description, title: 'Rekam Medis', subtitle: 'Lihat riwayat catatan konsultasi', onTap: () => push(context, MedicalRecordsScreen(user: user))),
          ActionTile(icon: Icons.receipt_long, title: 'Riwayat Pesanan', subtitle: 'Lihat status pesanan produk', onTap: () => push(context, OrdersScreen(user: user))),
          ActionTile(icon: Icons.notifications, title: 'Notifikasi', subtitle: 'Lihat notifikasi lokal', onTap: () => push(context, NotificationsScreen(user: user))),
        ],
      ),
    );
  }
